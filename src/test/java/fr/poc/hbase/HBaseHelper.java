...
//		configuration.set("hbase.cells.scanned.per.heartbeat.check", "10000");
		...
		utility.startMiniCluster(3, true);
		configuration.reloadConfiguration();
	...
	public void alterTable(String table, Consumer<HTableDescriptor> hTableDescriptorConfigurer) throws IOException {
		TableName tableName = TableName.valueOf(table);
		admin.disableTable(tableName);
		HTableDescriptor hTableDescriptor = admin.getTableDescriptor(tableName);
		hTableDescriptorConfigurer.accept(hTableDescriptor);
	...
	public void dumpResult(Result result) {
		for (Cell cell : result.rawCells()) {
			LOGGER.debug("Cell: {}, Value: {}", cell, Bytes.toString(cell.getValueArray(),
