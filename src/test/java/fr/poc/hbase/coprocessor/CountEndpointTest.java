...
public class CountEndpointTest {
	...
	public void testEndpoint() throws Throwable {
		...
		LOGGER.info("CountEndpointTest:testEndpoint executed in [{}]ms", System.currentTimeMillis() - start);
	}
	...
	public void testEndpointCombined() throws Throwable {
		...
		assertThat(combinedCount.getFirst()).as("Total row count")
				.isEqualTo(CountTestUtil.ROW_COUNT);
		assertThat(combinedCount.getSecond()).as("Total cell count")
				.isEqualTo(CountTestUtil.ROW_COUNT * CountTestUtil.COL_COUNT_PER_FAMILIES * CountTestUtil.FAMILIES.length);
		LOGGER.info("CountEndpointTest:testEndpointCombined executed in [{}]ms", System.currentTimeMillis() - start);
	}
	...
	public void testEndpointBatch() throws Throwable {
		...
		assertThat(new RowCountEndpointClient(table).getRowCountWithBatch()).as("Total row count")
				.isEqualTo(CountTestUtil.ROW_COUNT);
		LOGGER.info("CountEndpointTest:testEndpointBatch executed in [{}]ms", System.currentTimeMillis() - start);
	}
}
